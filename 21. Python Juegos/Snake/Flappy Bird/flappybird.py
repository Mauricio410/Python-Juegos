...
img_reset = Image.open('imagenes/reiniciar.png')
img_reset = ImageTk.PhotoImage(img_reset)

canvas = tk.Canvas(window, highlightthickness=0, bg= '#00bfff')
canvas.place(relwidth = 1, relheight=1)

text_score = canvas.create_text(50,50, text= '0', fill='black', font=('D3 Egoistism outline', 30))
...
mixer.music.load('audios/swoosh.wav')
...
def move_bird_key(event):
	...
	if not game_over:
		...
		mixer.music.load('audios/wing.wav')
		mixer.music.play(loops= 0)
...
def move_pipe():
	...
	if 0 < canvas.coords(pipe_down)[0]<160:
		...
		sound = mixer.Sound('audios/point.wav')
...
def reset_game():
	...
	game_over = False
	canvas.coords(bird, x,y)
	canvas.coords(pipe_top, 1500,-550)
	canvas.coords(pipe_down, 1500, 550)
...
def game_end():
	...
	mixer.music.load('audios/hit.wav')
	mixer.music.play(loops= 0)
	while mixer.music.get_busy():
		continue
	mixer.music.load('audios/die.wav')
	mixer.music.play(loops= 0)

lbl_game_over = tk.Label(window, text = 'Game Over !', font= ('D3 Egoistism outline', 30), fg='black', bg='#00bfff')
